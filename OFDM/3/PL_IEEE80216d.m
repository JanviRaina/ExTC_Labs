...
case 'ATNT', PLf=6*log10(fc/2e9); % Eq.(1.13)
             PLh=-10.8*log10(hrx/2); % Eq.(1.14)
case 'OKUMURA', PLf=6*log10(fc/2e9); % Eq.(1.13)
if hrx<=3, PLh=-10*log10(hrx/3); % Eq.(1.15)
else PLh=-20*log10(hrx/3);
end
case 'NO', PLf=0; PLh=0;
end
if Type=='A', a=4.6; b=0.0075; c=12.6; % Eq.(1.3)
...
for k=1:length(d)
if d(k)>d0_pr, PL(k) = A + 10*gamma*log10(d(k)/d0); % Eq.(1.18)
